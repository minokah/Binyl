/**
 * @brief Library to interface with the RFID Module. This file implements the RFIDModule class for interfacing with the RFID receiver.
 * @author Brandon Luu
 ...
 * Error = TAG_ERR
 */
int read_block(int addr, unsigned char *mess, int disp, string *str)
{
	unsigned char 	ch;
	int 	i; 
    ...
    if (read_tag_str(addr,mess) == TAG_OK)
    {
	   if (disp)
	   {
			...
			for(i = 0; i<32; i++)
			{
                ...
                readBuffer[count] = (char)mess[i];
                count++;

                if (count == 2) {
                    //printf("%c", (char)strtol(readBuffer, NULL, 16));
                    
                    char c = (char)stoi(readBuffer, NULL, 16);

                    // prevent null character from being added
                    if (c != '\0') *str += c;
                    count = 0;
                }
			}
...
string read_from_card(int block)
{
    int     		addr = -1, tmp;
    ...
    //p_printf(YELLOW, "Read: ");
    //addr = get_block_number(max_blocks-1,0);

	// read block and display content
    string result = "";
	tmp = read_block(block, buff, 1, &result);
    //PcdHalt(); 

    //tmp = read_block(9, buff, 1, &result);
...
int get_card_info()
{
    ...
    do
    {
        ...
        if (cnt++ > 3){                 // display keep alive
            ...
            // Re-enable this to show if the card is reading or not!
            //p_printf(BLUE,".");
        }
...
// Destructor, close out module
RFIDModule::~RFIDModule() {
    close_out(0);
}

// Get the data in a block
string RFIDModule::get(int block) {
    string result = read_from_card(block);
    //if (read_from_card() != TAG_OK) close_out(1);
    ...
    //p_printf(YELLOW,"Next card reading in 5 seconds...\n");  
    //sleep(5);
...
RFIDModule& RFIDModule::getInstance() {
    static RFIDModule instance;
    return instance;
}